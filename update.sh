#!/bin/bash
# update.sh — Atualiza dados da Aliadacont e faz redeploy
cd "$(dirname "$0")"

echo "=== Aliadacont BI Update — $(date) ==="
...
if git diff --cached --quiet; then
  ...
  curl -s "http://187.77.238.125:8000/api/v1/deploy?uuid=q1474bbwbxmcx02btow5kbzc&force=true" \
